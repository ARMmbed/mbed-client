...
## Example use cases (cached)

### Integers from mbed Client to mbed Device Server
...
int size = sprintf(buffer,"%d",value);
instance->set_value((const uint8_t*)buffer, size);
instance->set_max_age(15) ; // This is time to live for resource in Server cache (in seconds)
```

**Response on the REST API**:

```
{"async-response-id ... { "id" : "142340106#f4b93d6e-4652-4874-82e4-41a3ced0cd56@b02ac40a-a501-44d9-be29-e6e0e5303f85/Test/0/D", "status" : 200, "payload" : "MQ==", "ct" : "text/plain", "max-age" : 15 } 
```

- Base64 encoded value of the payload -> `MQ==`
- Base64 decoded value of the payload -> `1`

### A float value from mbed Client to mbed Device Server

**Float (from the application)**:

```
float value = 1.0232f;
M2MResource* instance = objinst->create_dynamic_resource(“D", “FloatType“,M2MResourceInstance::FLOAT, true);
char buffer[20];
...
**Response on the REST API**:
 ...
 ```
 {"async-response-id ... { "id" : "143026051#f4b93d6e-4652-4874-82e4-41a3ced0cd56@b02ac40a-a501-44d9-be29-e6e0e5303f85/Test/0/D", "status" : 200, "payload" : "MS4wMjMyMDA=", "ct" : "text/plain", "max-age" : 15 } 
 ```
 
- Base64 encoded value of the payload -> `MS4wMjMyMDA=`
- Base64 decoded value of the payload -> `1.023200`

### A boolean value from mbed Client to mbed Device Server

**Boolean (from the application)**:

```
bool value = true;
M2MResource* instance = objinst->create_dynamic_resource(“D", “BooleanType“,M2MResourceInstance::BOOLEAN, true);
char buffer[20];
int size = sprintf(buffer,"%d", value);
instance->set_value((const uint8_t*)buffer, size);
instance->set_max_age(15) ; // This is time to live for resource in Server cache (in seconds)
...
```
{"async-response-id": ... { "id" : "1218383323#f4b93d6e-4652-4874-82e4-41a3ced0cd56@b02ac40a-a501-44d9-be29-e6e0e5303f85/Test/0/D", "status" : 200, "payload" : "MQ==", "ct" : "text/plain", "max-age" : 15 }
```

- Base64 encoded value of the payload -> `MQ==`
- Base64 decoded value of the payload -> `1`

### A string from mbed Client to mbed Device Server

**String (from the application)**:

```
M2MResource* instance = objinst->create_dynamic_resource(“D", “StringType“,M2MResourceInstance::STRING, true);
char buffer[10] = “Test Data”;
instance->set_value((const uint8_t*)buffer, 10);
instance->set_max_age(15) ; // This is time to live for resource in Server cache (in seconds)
```
...
```
{"async-response-id": ... { "id" : "1219658892#f4b93d6e-4652-4874-82e4-41a3ced0cd56@b02ac40a-a501-44d9-be29-e6e0e5303f85/Test/0/D", "status" : 200, "payload" : "VGVzdCBEYXRhAA==", "ct" : "text/plain", "max-age" : 15 }
```

- Base64 encoded value of the payload -> `VGVzdCBEYXRhAA==`
- Base64 decoded value of the payload -> `Test Data`
 
### An opaque value from mbed Client to mbed Device Server

**Opaque (from the application)

```
M2MResource* instance = objinst->create_dynamic_resource(“D", “OpaqueType“,M2MResourceInstance::OPAQUE, true);
uint8_t buffer[12] = “Opaque Data”;
instance->set_value(buffer, 12);
instance->set_max_age(15) ; // This is time to live for resource in Server cache (in seconds)
```
...
```
{"async-response-id": ... { "id" : "1220357914#f4b93d6e-4652-4874-82e4-41a3ced0cd56@b02ac40a-a501-44d9-be29-e6e0e5303f85/Test/0/D", "status" : 200, "payload" : "T3BhcXVlIERhdGEA", "ct" : "application/octet-stream", "max-age" : 15 }
```
